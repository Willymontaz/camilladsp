#!/usr/bin/env python3
"""
Polyphase listening-quality benchmark for CamillaDSP.

This bench is intentionally narrower than the historical M4/GPU experiment:
it only exercises the current `Polyphase` resampler over the common listening
paths 44.1/48/88.2 kHz -> 96 kHz. The engine derives its polyphase branch count
from the rate pair (320 for 44.1 kHz, 160 for 88.2 kHz, 2 for 48 kHz), so the
only knob swept here is the tap count.
...
def branch_count(src_rate: int) -> int:
    """Polyphase branches the engine derives for `src_rate` -> DST_RATE."""
    return DST_RATE // math.gcd(src_rate, DST_RATE)
...
class RunRecord:
    ...
    def csv_row(self) -> dict[str, object]:
        ...
        return {
            ...
            "signal": self.signal,
            "taps": self.spec.taps,
            "oversampling": branch_count(self.rate),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the Polyphase listening-quality bench.")
