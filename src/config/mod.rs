...
pub enum Resampler {
    ...
    /// character. The ratio is fixed at construction; use `AsyncSinc` if
    /// `rate_adjust: true` is required.
    ///
    /// Intersample-peak protection is no longer wired here: use the master-bus
    /// `TruePeak` processor placed last in the pipeline, which also guards the
    /// peaks generated by post-resampler stages (e.g. `TubeStage`).
    Polyphase {
...
/// Parameters for the master-bus [TruePeak](Processor::TruePeak) limiter: a 4x
/// oversampled true-peak detector plus lookahead limiter that caps the DAC feed
/// at `ceiling_dbfs`. Placed last in the pipeline to guard intersample peaks
/// created by every upstream stage (resampler, TubeStage, EQ, gain).
#[derive(Clone, Debug, Serialize, Deserialize, PartialEq)]
#[serde(deny_unknown_fields)]
pub struct TruePeakParameters {
    pub channels: usize,
    /// Linear ceiling in dBFS the true peak is held at (default -1.0).
    #[serde(default)]
    pub ceiling_dbfs: Option<f64>,
    /// One-pole gain-recovery time constant in ms (default 50.0).
    #[serde(default)]
    pub release_ms: Option<f64>,
}

impl TruePeakParameters {
    pub fn ceiling_dbfs(&self) -> f64 {
        self.ceiling_dbfs.unwrap_or(-1.0)
    }
