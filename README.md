...
Intersample-peak protection is handled by the master-bus
[`TruePeak`](#truepeak) processor placed last in the pipeline, rather than by
the resampler itself. Placing it at the end of the chain guards the peaks
generated by every post-resampler stage (`TubeStage`, EQ, gain) — not just the
resampler output. A full example configuration is provided at
...
### TruePeak
The "TruePeak" processor is a master-bus true-peak (intersample-peak) limiter.
It runs the signal through a 4x oversampled true-peak detector and a lookahead
peak limiter, capping the true peak of the output at a configurable ceiling to
protect a downstream DAC's reconstruction filter from clipping.

Placed **last** in the pipeline, it guards the intersample peaks generated by
*every* upstream stage — the resampler, but also gain, EQ and non-linear stages
such as `TubeStage` that create new peaks after resampling.

The gain is linked across all channels (the same attenuation is applied to
every channel, preserving imaging), and the currently applied attenuation in dB
is surfaced on the websocket via `GetTruePeakAttenuation` (`0.0` means no
limiting, negative means the limiter is active), so a monitor UI can show
DAC-protect headroom. The lookahead adds a small fixed latency (16 samples at
the internal 4x rate).
...
```
  ...
  Parameters:
  * `channels`: number of channels, must match the number of channels of the pipeline where the limiter is inserted (max 32).
  * `ceiling_dbfs`: the true-peak ceiling in dBFS the output is held at. Must be `<= 0.0`. Optional, defaults to -1.0.
  * `release_ms`: one-pole gain-recovery time constant in milliseconds for restoring gain after a peak. Optional, defaults to 50.0.
